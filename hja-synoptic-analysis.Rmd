...
fticrms_wide <- t(fticrms[,15:76])
rownames(fticrms_wide)
colnames(fticrms_wide) <- fticrms$`Measured Ionic Mass (m/z)`
ms_pca <- rda(fticrms_wide)
screeplot(ms_pca)
biplot(ms_pca)
eigenvals(ms_pca)[1]/sum(eigenvals(ms_pca))
eigenvals(ms_pca)[2]/sum(eigenvals(ms_pca))

ms_toplot <- fticrms[,c(1,15:76)] %>% 
  pivot_longer(cols = !(`Measured Ionic Mass (m/z)`),
               names_to = "Site Code", 
               values_to = "Peaks")

ms_toplot %>% 
  ggplot(aes(x = `Measured Ionic Mass (m/z)`, y = `Site Code`, height = Peaks)) + 
  geom_ridgeline() + 
  scale_y_discrete(expand = c(0.01, 0)) + 
  scale_x_continuous(expand = c(0.01, 0)) + 
  theme_ridges() + theme(legend.position = "n")
```
