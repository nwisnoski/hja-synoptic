...
env.aquatic <- read_csv("data/hja-env_data_clean.csv",
                      col_types = cols(
                        ...
                        Time = col_time(format = "%H:%M")), n_max = 186)
env.aquatic <- env.aquatic %>% 
...
OTUs <- OTUs[which(rowSums(OTUs)>3000),]
...
# Plot 
hja_synoptic_microbes_ordination <- left_join(rownames_to_column(as.data.frame(pcdims), var = "Sample.Name"), design) %>% 
...
ggsave(filename = here("figures/hja_synoptic_microbes_ordination.pdf"), 
       plot = hja_synoptic_microbes_ordination, dpi = 600, width = 6, height = 6, units = "in")
```
...
DOM_env_df <- left_join(rownames_to_column(as.data.frame(pcdims), var = "Site.Code"), 
          env.aquatic %>% filter(`Sample Type` == "Sediment") %>% rename(Site.Code = `Site Code`),
          by = "Site.Code")

# Plot 
hja_synoptic_DOM_ordination <- DOM_env_df %>% 
  ggplot(aes(x = PCoA1, y = PCoA2, color = as.factor(Stream_Order))) + 
  geom_point(alpha = 0.5) + 
  #stat_ellipse() +
  coord_fixed() +
  geom_text_repel(aes(label = Site.Code), show.legend = F) +
  labs(x = paste0("PCoA1 (", round(var1,2),"%)"),
       y = paste0("PCoA2 (", round(var2,2),"%)"), 
       color = "Stream Order") +
  scale_color_viridis_d(begin = 0.1, end = 0.9, direction = -1)
hja_synoptic_DOM_ordination

ggsave(filename = here("figures/hja-synoptic-ordination-dom.pdf"), 
       plot = hja_synoptic_DOM_ordination, dpi = 600, width = 6, height = 6, units = "in")

```

RDA microbes and molecules
```{r}
env_physical_features <- DOM_env_df %>% 
  select(Site.Code, Drainage_area_ha, Stream_Order, 
         Valley_Distance_to_outlet_m, `Segment_Valley_Avg_Slope_m/m`,
         `Segment_Stream_Avg_Slope_m/m`, Segment_Valley_Avg_Width_m,
         `Segment_Sinuosity_m/m`)

env_physical_features_mod <- scale(env_physical_features[-1])
dom_rda <- rda(hja_peaks ~ ., data = as.data.frame(env_physical_features_mod))
plot(dom_rda)

sediment_samples <- design %>% 
  filter(Site.Code %in% env_physical_features$Site.Code,
         Sample.Type == "Sediment") %>% 
  select(Sample.Name) %>% as_vector()
OTUs.hel.sed <- OTUs.hel[which(rownames(OTUs.hel) %in% sediment_samples),]

rda(OTUs.hel.sed ~ ., data = env_physical_features)
```
...
DOM_diversity <- (colSums(data.frame(sed_peaks_pa$e_data[,-1])))
sed_otus <- OTUs[sed_samples_in_ICRMS,]
mic_diversity <- exp(diversity(sed_otus))

plot(DOM_diversity, mic_diversity)
```
...
aquatic.dat <- inner_join(design[which(design$Sample.Type == "Stream"),], (env.aquatic))
aquatic.geo <- sqrt(dist(aquatic.dat$Stream_Centerline_UTM_Zone_10N_X_m)^2 + dist(aquatic.dat$Stream_Centerline_UTM_Zone_10N_Y_m)^2)
...
sed.geo <- sqrt(dist(aquatic.dat$Stream_Centerline_UTM_Zone_10N_X_m)^2 + dist(aquatic.dat$Stream_Centerline_UTM_Zone_10N_Y_m)^2)
